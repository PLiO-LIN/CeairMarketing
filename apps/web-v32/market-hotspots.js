(function () {
  ...
  function inject() {
    ...
    if (!window.ceairHotspotTimer) {
      window.ceairHotspotTimer = window.setInterval(() => {
  ...
  async function collect() { const button=q('[data-hotspot-ingest]'); if(button) {button.disabled=true;button.textContent='正在获取热点...';} try { const result=await api('/api/market-hotspots/collect',{method:'POST',body:JSON.stringify({process_with_agent:true,sources:[{name:'Google News 航旅热点',url:'https://news.google.com/rss/search?q=航空+旅游+中国',source_type:'rss',max_items:20},{name:'行业资讯热点',url:'https://www.travelweekly.com/rss/news.xml',source_type:'rss',max_items:20}]})}); alert(`热点获取已提交：新增 ${result.created||0} 条，Agent 正在分析`); await load(); } catch(e) { alert(e.message); } finally {if(button){button.disabled=false;button.innerHTML='<i data-lucide="radar"></i>获取最新热点';if(window.lucide)window.lucide.createIcons();}} }
  function bind() { q('#hotspotSearch')?.addEventListener('input', () => load()); q('#hotspotStatusFilter')?.addEventListener('change', () => load()); q('[data-hotspot-refresh]')?.addEventListener('click', load); q('[data-hotspot-ingest]')?.addEventListener('click', collect); q('#hotspotList')?.addEventListener('click', async e => { const row = e.target.closest('[data-id]'); if (row) { const item = await api(`/api/market-hotspots/${row.dataset.id}`); detail(item); } }); q('#hotspotDetail')?.addEventListener('click', async e => { const b = e.target.closest('button'); if (!b) return; try { if (b.dataset.review) await api(`/api/market-hotspots/${b.dataset.id}/review`, {method:'POST', body:JSON.stringify({decision:b.dataset.review, note:'营销人员在线确认'})}); if (b.dataset.opportunity) await api(`/api/market-hotspots/${b.dataset.opportunity}/opportunity`, {method:'POST', body:JSON.stringify({})}); if (b.dataset.process) await api(`/api/market-hotspots/${b.dataset.process}/process`, {method:'POST'}); await load(); alert('操作已完成'); } catch (e) { alert(e.message); } }); }
  document.addEventListener('DOMContentLoaded', inject); if (document.readyState !== 'loading') inject(); window.ceairHotspots = {open, load, collect};
}());
