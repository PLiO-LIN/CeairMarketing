...
async function collect() {
    const button = q('[data-hotspot-ingest]');
    ...
    try {
      ...
      if (first) { const item = await api(`/api/market-hotspots/${first.id}`); detail(item); }
    } catch (e) { alert(e.message); }
    finally { if (button) { button.disabled = false; button.innerHTML = '<i data-lucide="radar"></i>获取最新热点'; if (window.lucide) window.lucide.createIcons(); } }
