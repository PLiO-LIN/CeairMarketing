(function () {
  ...
  let session = null;
  let tenantId = null;
  let tenantData = { campaigns: [], graph: { nodes: [], edges: [] }, imports: [], pipelines: [], providers: [], domains: [], runs: [], mineru: null, opportunities: [], audienceTags: [], audiencePackages: [], personaDimensions: [], personaSegments: [], productPackages: [], contentAssets: [], audienceSnapshots: [], approvals: [], executionBatches: [], documents: [] };
...
<table class="table" id="userTable">
  ...
  function renderAudienceStructure(){
    ...
    const dimensions=tenantData.personaDimensions||[], segments=tenantData.personaSegments||[];
    const typeMap=new Map(); segments.forEach(item=>{const key=item.primary_persona_code||'other'; if(!typeMap.has(key))typeMap.set(key,{name:item.primary_persona_name||'\u672a\u5206\u7c7b\u753b\u50cf',count:0}); typeMap.get(key).count++;});
    const typeCards=[...typeMap.values()].slice(0,8).map(item=>'<div class="persona-type"><b>'+escapeHtml(displayText(item.name,'\u672a\u5206\u7c7b\u753b\u50cf'))+'</b><span>'+item.count+'\u4e2a\u7ec6\u5206\u5ba2\u7fa4</span></div>').join('');
    const dimensionRows=dimensions.slice(0,12).map(item=>'<tr><td><strong>'+escapeHtml(displayText(item.field_name,'\u672a\u547d\u540d\u7ef4\u5ea6'))+'</strong><small>'+escapeHtml(displayText(item.field_code,'\u65e0\u7f16\u7801'))+'</small></td><td>'+escapeHtml(displayText(item.module_name,'\u57fa\u7840\u753b\u50cf'))+'</td><td>'+escapeHtml(displayText(item.data_type,'\u6587\u672c'))+'</td><td>'+escapeHtml(displayText(item.update_frequency,'\u6309\u65e5\u66f4\u65b0'))+'</td><td><span class="status good">\u53ef\u7528\u4e8e\u5708\u9009</span></td></tr>').join('');
    panel.innerHTML='<div class="persona-summary"><div class="persona-summary-head"><div><b>\u753b\u50cf\u76ee\u5f55</b><span>\u5185\u90e8\u753b\u50cf\u63a5\u53e3\u540c\u6b65 ? '+dimensions.length+'\u4e2a\u7ef4\u5ea6 ? '+segments.length+'\u4e2a\u7ec6\u5206\u5ba2\u7fa4</span></div><button class="btn" data-action="refreshAudienceCatalog"><i data-lucide="refresh-cw"></i>\u540c\u6b65\u753b\u50cf</button></div><div class="persona-types">'+(typeCards||'<span class="muted">\u6682\u65e0\u753b\u50cf\u7c7b\u578b</span>')+'</div></div><div class="audience-builder"><div class="audience-layer output"><b>\u5ba2\u7fa4\u5305</b><small>\u753b\u50cf\u7ec4\u5408\u7ed3\u679c\uff0c\u53ef\u5f15\u7528\u5230\u8425\u9500\u6d3b\u52a8</small><div class="audience-packages">'+(packages.length?packages.map(item=>'<div class="audience-package"><strong>'+escapeHtml(displayText(item.name,'\u672a\u547d\u540d\u5ba2\u7fa4\u5305'))+'</strong><span>'+Number(item.estimated_size||0).toLocaleString('zh-CN')+'\u4eba ? '+escapeHtml(item.selection_mode==='ai-selection'?'AI\u5708\u9009':'\u6807\u7b7e\u7ec4\u5408')+'</span><button class="btn" data-audience-snapshot="'+item.id+'">\u51bb\u7ed3\u5feb\u7167</button></div>').join(''):'<span class="muted">\u70b9\u51fb\u201c\u65b0\u5efa\u5ba2\u7fa4\u201d\u521b\u5efa\u7b2c\u4e00\u4e2a\u5ba2\u7fa4\u5305</span>')+'</div></div><div class="audience-connector">\u7531\u753b\u50cf\u7ef4\u5ea6\u4e0e\u6807\u7b7e\u6761\u4ef6\u6784\u6210</div><div class="audience-layer"><b>\u5ba2\u7fa4\u6807\u7b7e</b><small>\u53ef\u590d\u7528\u7cfb\u7edf\u6807\u7b7e\uff0c\u4e5f\u53ef\u5728\u5708\u9009\u65f6\u8ffd\u52a0\u6761\u4ef6</small><div class="audience-chips">'+(tags.length?tags.slice(0,30).map(item=>'<span class="tag blue">'+escapeHtml(displayText(item.name,'\u672a\u547d\u540d\u6807\u7b7e'))+'</span>').join(''):'<span class="muted">\u6682\u65e0\u540c\u6b65\u6807\u7b7e</span>')+'</div></div></div><div class="snapshot-strip"><b>\u5df2\u51bb\u7ed3\u5feb\u7167</b><span>'+(snapshots.length?snapshots.slice(0,5).map(item=>escapeHtml(item.external_id)+' ? '+escapeHtml(item.version)).join('?'):'\u6682\u65e0\u6d3b\u52a8\u5feb\u7167')+'</span></div><div class="persona-dimension-table"><div class="subsection-head"><b>\u753b\u50cf\u7ef4\u5ea6\u660e\u7ec6</b><span>\u5c55\u793a\u753b\u50cf\u5b9a\u4e49\uff0c\u4e0d\u5c55\u793a\u65c5\u5ba2\u660e\u7ec6</span></div><table class="table"><tr><th>\u7ef4\u5ea6</th><th>\u753b\u50cf\u6a21\u5757</th><th>\u7c7b\u578b</th><th>\u66f4\u65b0\u9891\u7387</th><th>\u4f7f\u7528\u72b6\u6001</th></tr>'+(dimensionRows||'<tr><td colspan="5" class="muted">\u6682\u65e0\u753b\u50cf\u7ef4\u5ea6</td></tr>')+'</table></div>';
    if(window.lucide)lucide.createIcons();
  }
