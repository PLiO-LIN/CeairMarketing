...
<em>${item.id===activeTenant().id?'当前':'切换'}</em>
  ...
  function mountMarketingAssistantV2(){
    ...
    root.innerHTML='<button class="assistant-fab" title="\u6253\u5f00\u8425\u9500\u52a9\u624b"><i data-lucide="bot"></i><span>\u8425\u9500\u52a9\u624b</span><em>AI</em></button><section class="assistant-panel" hidden><header><div class="assistant-title"><span class="assistant-avatar"><i data-lucide="sparkles"></i></span><div><b>AI\u8425\u9500\u52a9\u624b</b><small>\u4e1c\u822a\u8425\u9500\u534f\u540c ? Harness</small></div></div><div class="assistant-header-actions"><span class="assistant-live"><i></i>\u5728\u7ebf</span><button class="icon-btn" data-assistant-close aria-label="\u5173\u95ed"><i data-lucide="x"></i></button></div></header><div class="assistant-context"><span><i data-lucide="database"></i>\u77e5\u8bc6\u4e2d\u5fc3</span><span><i data-lucide="workflow"></i>\u516d\u5927\u667a\u80fd\u57df</span><span><i data-lucide="shield-check"></i>\u4eba\u5de5\u786e\u8ba4</span></div><div class="assistant-messages"><div class="assistant-message assistant"><div class="assistant-mark"><i data-lucide="sparkles"></i></div><div><b>\u4f60\u597d\uff0c\u6211\u662f\u4e1c\u822a AI \u8425\u9500\u52a9\u624b</b><p>\u53ef\u4ee5\u68c0\u7d22\u77e5\u8bc6\u4e2d\u5fc3\u3001\u672c\u4f53\u5173\u7cfb\uff0c\u5206\u6790\u673a\u4f1a\u4e0e\u5ba2\u7fa4\uff0c\u5339\u914d\u4ea7\u54c1\u5305\u5e76\u67e5\u770b\u6d3b\u52a8\u72b6\u6001\u3002</p><div class="assistant-suggestions"><button data-assistant-suggest="\u5f53\u524d\u6709\u54ea\u4e9b\u9ad8\u4ef7\u503c\u8425\u9500\u673a\u4f1a\uff1f">\u9ad8\u4ef7\u503c\u673a\u4f1a</button><button data-assistant-suggest="\u67e5\u8be2\u8fd1\u671f\u6570\u636e\u5904\u7406\u4efb\u52a1">\u6d41\u6c34\u7ebf\u8fdb\u5ea6</button></div></div></div></div><form><input name="message" placeholder="\u8f93\u5165\u8425\u9500\u95ee\u9898\u6216\u64cd\u4f5c\u6307\u4ee4" autocomplete="off"><button class="btn primary" aria-label="\u53d1\u9001"><i data-lucide="arrow-up"></i></button></form></section>';
    document.body.appendChild(root);
    const fab=q('.assistant-fab',root),panel=q('.assistant-panel',root),messages=q('.assistant-messages',root),form=q('form',root),input=q('input',root);
    const traceLabel={"harness/context-loaded":"\u52a0\u8f7d\u4e1a\u52a1\u4e0a\u4e0b\u6587","harness/model-started":"\u6a21\u578b\u601d\u8003\u4e2d","harness/model-finished":"\u6a21\u578b\u8f93\u51fa\u5b8c\u6210","harness/tool-started":"\u8c03\u7528\u4e1a\u52a1\u5de5\u5177","harness/tool-finished":"\u5de5\u5177\u8fd4\u56de\u7ed3\u679c","harness/json-parsed":"\u89e3\u6790\u51b3\u7b56\u7ed3\u679c","model/provider-fallback":"\u5207\u6362\u5907\u7528\u6a21\u578b"};
    const addTrace=(traceBox,item)=>{const label=traceLabel[item.event]||item.event||'Agent event';const detail=Object.entries(item).filter(([key])=>!['event','timestamp'].includes(key)).map(([key,value])=>key+': '+(typeof value==='string'?value:JSON.stringify(value))).join(' ? ');traceBox.insertAdjacentHTML('beforeend','<div class="assistant-trace-row"><span class="assistant-trace-dot"></span><div><b>'+escapeHtml(label)+'</b><small>'+new Date(item.timestamp||Date.now()).toLocaleTimeString('zh-CN')+'</small><p>'+escapeHtml(detail||'\u5df2\u5b8c\u6210')+'</p></div></div>');traceBox.parentElement.scrollTop=traceBox.parentElement.scrollHeight;};
    const send=async message=>{if(!message||form.dataset.busy)return;form.dataset.busy='1';input.disabled=true;messages.insertAdjacentHTML('beforeend','<div class="assistant-message user"><div>'+escapeHtml(message)+'</div></div>');const wrap=document.createElement('div');wrap.className='assistant-message assistant live-message';wrap.innerHTML='<div class="assistant-mark"><i data-lucide="sparkles"></i></div><div class="assistant-live-body"><div class="assistant-streaming"><span></span><span></span><span></span><b>\u6b63\u5728\u5206\u6790\u4e1a\u52a1\u4e0a\u4e0b\u6587</b></div><details class="assistant-trace-details" open><summary><i data-lucide="route"></i><b>Agent\u6267\u884c\u8fc7\u7a0b</b><span>\u5b9e\u65f6</span></summary><div class="assistant-trace-list"></div></details><p class="assistant-answer"></p></div>';messages.appendChild(wrap);if(window.lucide)lucide.createIcons();const traceList=q('.assistant-trace-list',wrap),answer=q('.assistant-answer',wrap),streaming=q('.assistant-streaming',wrap);try{const response=await fetch(`${mount}/api/agent-chat/stream`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session?.access_token||''}`,'X-Tenant-ID':String(activeTenant()?.id||'')},body:JSON.stringify({message,domain_id:'marketing-copilot',history:[]})});if(!response.ok)throw new Error((await response.json().catch(()=>({}))).detail||'\u667a\u80fd\u4f53\u8fde\u63a5\u5931\u8d25');const reader=response.body.getReader(),decoder=new TextDecoder();let buffer='';while(true){const part=await reader.read();if(part.done)break;buffer+=decoder.decode(part.value,{stream:true});const frames=buffer.split('\n\n');buffer=frames.pop()||'';for(const frame of frames){const event=(frame.match(/^event: (.+)$/m)||[])[1]||'message';const data=(frame.match(/^data: (.+)$/m)||[])[1];if(!data)continue;const value=JSON.parse(data);if(event==='trace'){addTrace(traceList,value);streaming.querySelector('b').textContent=value.event==='harness/model-started'?'\u6a21\u578b\u6b63\u5728\u751f\u6210\u51b3\u7b56':'\u6b63\u5728\u6267\u884c'+(traceLabel[value.event]||'\u4e1a\u52a1\u6b65\u9aa4');}if(event==='token'){answer.textContent+=value.text;}if(event==='complete'){streaming.remove();const summary=q('.assistant-trace-details summary',wrap);if(summary)summary.querySelector('span').textContent='\u5df2\u5b8c\u6210';if(value.sources?.length)answer.insertAdjacentHTML('beforeend','<div class="assistant-sources"><i data-lucide="quote"></i>\u5df2\u5f15\u7528 '+value.sources.length+' \u6761\u4e1a\u52a1\u4f9d\u636e</div>');}if(event==='error'){streaming.remove();answer.textContent=value.message||'\u667a\u80fd\u4f53\u8fd0\u884c\u5931\u8d25';}}}streaming?.remove();}catch(err){streaming?.remove();answer.textContent='\u8bf7\u6c42\u5931\u8d25\uff1a'+(err.message||'\u8bf7\u7a0d\u540e\u91cd\u8bd5');}finally{form.dataset.busy='';input.disabled=false;input.focus();messages.scrollTop=messages.scrollHeight;if(window.lucide)lucide.createIcons();}};
    fab.addEventListener('click',()=>{panel.hidden=!panel.hidden;if(!panel.hidden)input.focus();});q('[data-assistant-close]',root).addEventListener('click',()=>panel.hidden=true);qa('[data-assistant-suggest]',root).forEach(button=>button.addEventListener('click',()=>send(button.dataset.assistantSuggest)));form.addEventListener('submit',e=>{e.preventDefault();const value=input.value.trim();input.value='';send(value);});if(window.lucide)lucide.createIcons();
  }
