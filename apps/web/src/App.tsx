...
function App() {
  ...
  async function deletePipeline(job: DataPipelineJob) {
    if (!confirm(`确认删除“${job.file_name}”及其生成的知识、图谱对象和处理记录吗？`)) return
    try {
      await request(`/api/data-pipelines/${job.id}`, { method: 'DELETE' })
      setPipelineDetail(null); setMessage('文档及其处理结果已删除'); await loadTenantData()
  ...
  const nav = [
    ...
    ['chat', '营销智能助手', MessageSquare], ['ontology', '知识中心', BookOpen], ['imports', '数据处理流水线', FileUp], ['models', '模型配置', ServerCog],
  ...
  return <div className="app-shell production-shell">
    ...
    <main><header className="topbar"><div><span>东方航空</span><ChevronRight size={14}/><b>{nav.find(([id]) => id === view)?.[1]}</b></div><div className="top-actions"><select value={activeTenant?.id} onChange={(event) => switchTenant(Number(event.target.value))}>{session.tenants.map((tenant) => <option key={tenant.id} value={tenant.id}>{tenant.name}</option>)}</select><label><Search/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索活动、产品包或实体"/></label><button title="刷新" onClick={() => void loadTenantData()}><RefreshCw/></button></div></header>
      ...
      <div className="page">
        ...
        {view === 'imports' && <DataPipelineWorkspace jobs={pipelines} detail={pipelineDetail} onDetail={setPipelineDetail} onUpload={uploadData} onReview={reviewPipeline} onDelete={deletePipeline}/>}
...
function GraphWorkspace({ graph, stats }: { graph: MarketingGraph; stats: GraphStats }) {
  ...
  return <><div className="page-head"><div><h1>知识中心</h1><p>统一管理知识文档、业务对象、本体关系与来源证据，为智能体提供可信上下文</p></div><div className="graph-summary"><span>{stats.entity_count} 对象</span><span>{stats.relation_count} 关系</span><span>{stats.source_count} 来源</span></div></div><div className="ontology-panel dynamic-ontology"><GraphCanvas graph={graph} onSelect={setSelected}/><aside className="relation-ledger"><div className="panel-title"><h2>{selected ? '知识对象详情' : '知识类型分布'}</h2><span>本体关系视图</span></div>{selected ? <div className="entity-detail"><em>{selected.type}</em><h3>{selected.label}</h3><p>{selected.id}</p><dl><dt>数据来源</dt><dd>{selected.source}</dd><dt>可信度</dt><dd>{Math.round(selected.confidence * 100)}%</dd>{Object.entries(selected.attributes).map(([key, value]) => <><dt key={`${key}-k`}>{key}</dt><dd key={`${key}-v`}>{typeof value === 'object' ? JSON.stringify(value) : String(value)}</dd></>)}</dl></div> : Object.entries(stats.entity_types).map(([type, count]) => <div className="type-row" key={type}><span>{type}</span><b>{count}</b></div>)}</aside></div></>
}
...
function Models({ providers, canManage, request, onAdd, onEdit, onAction }: { providers: ModelProvider[]; canManage: boolean; request: <T>(path: string, init?: RequestInit, form?: boolean) => Promise<T>; onAdd: () => void; onEdit: (provider: ModelProvider) => void; onAction: (provider: ModelProvider, action: 'test' | 'default' | 'delete') => void }) {
  ...
  const clean = (value: string, fallback: string) => !value.trim() || /[?\uFFFD]/.test(value) ? fallback : value
  return <><div className="page-head"><div><h1>模型与智能体配置</h1><p>参考对话产品的提供商配置方式，统一管理接口、可用模型、默认路由和调用用量</p></div>{canManage && <button className="primary" onClick={onAdd}><Plus/>添加提供商</button>}</div><div className="model-console"><aside className="panel provider-list"><div className="panel-title"><h2>模型提供商</h2><span>{providers.length} 个</span></div>{providers.map((provider) => <button className={selected?.id === provider.id ? 'selected' : ''} onClick={() => void loadProvider(provider)} key={provider.id}><span><ServerCog/></span><div><b>{clean(provider.display_name, '未命名提供商')}</b><small>{clean(provider.model_name, '未识别模型')}</small></div>{provider.is_default && <em>默认</em>}</button>)}</aside><section className="panel provider-detail">{selected ? <><header className="provider-detail-head"><div><span><ServerCog/></span><div><h2>{clean(selected.display_name, '未命名提供商')}</h2><p>{selected.provider_type} · {selected.base_url || '平台内置模型'}</p></div></div>{canManage && <div><button className="secondary" onClick={() => onAction(selected, 'test')}><Radio/>连接测试</button><button className="secondary" onClick={() => onEdit(selected)}><Settings2/>编辑</button></div>}</header><div className="provider-metrics"><div><small>默认模型</small><b>{clean(selected.model_name, '未识别模型')}</b></div><div><small>调用次数</small><b>{usage?.request_count ?? '—'}</b></div><div><small>总 Token</small><b>{usage?.total_tokens?.toLocaleString() ?? '—'}</b></div><div><small>凭证状态</small><b>{selected.api_key_configured ? '已安全配置' : selected.provider_type === 'mock' ? '无需凭证' : '未配置'}</b></div></div><div className="provider-sections"><section><div className="section-heading"><div><b>可用模型</b><span>从提供商接口实时读取</span></div><button className="secondary" onClick={() => void loadProvider(selected)}><RefreshCw className={loading ? 'spin' : ''}/>刷新</button></div><div className="model-list">{models.map((model) => <div key={model.id}><span><Bot/></span><b>{clean(model.id, '未识别模型')}</b><small>{clean(model.owned_by, '模型提供商')}</small>{model.id === selected.model_name && <em>当前</em>}</div>)}{!models.length && <p>点击刷新读取可用模型列表。</p>}</div></section><section><div className="section-heading"><div><b>用量分布</b><span>按模型统计调用和 Token</span></div></div><div className="usage-list">{usage?.by_model?.map((item) => <div key={item.model_name}><b>{clean(item.model_name, '未识别模型')}</b><span>{item.request_count} 次</span><em>{item.total_tokens.toLocaleString()} tokens</em></div>)}{!usage?.by_model?.length && <p>暂无调用记录。</p>}</div></section></div></> : <Empty icon={ServerCog} title="尚未配置模型" text="添加一个 OpenAI 兼容提供商后即可运行数据处理和营销智能体。"/>}</section></div></>
}
...
function DataPipelineWorkspace({ jobs, detail, onDetail, onUpload, onReview, onDelete }: { jobs: DataPipelineJob[]; detail: DataPipelineJob | null; onDetail: (job: DataPipelineJob | null) => void; onUpload: (type: string, file: File) => void; onReview: (job: DataPipelineJob, decision: 'approve' | 'reject') => void; onDelete: (job: DataPipelineJob) => void }) {
  const [dragging, setDragging] = useState(false)
  const [selected, setSelected] = useState<DataPipelineJob | null>(detail)
  useEffect(() => { setSelected(detail) }, [detail])
  const stageLabels: Record<string, string> = { queued: '排队', received: '接收检查', extracting: '解析结构', extracted: '解析完成', 'knowledge-persisting': '知识入库', 'knowledge-ready': '知识就绪', 'agent-processing': '智能体处理', 'agent-output': '智能体输出', 'semantic-validation': '语义校验', 'awaiting-confirmation': '待人工确认', 'ontology-persisting': '更新本体', 'ontology-updated': '本体已更新', rejected: '已驳回', failed: '失败' }
  ...
  const preview = (value: unknown) => typeof value === 'string' ? value : value ? JSON.stringify(value, null, 2) : ''
  return <>
    ...
    <div className="pipeline-layout">
      ...
      <section className="panel pipeline-detail"><div className="panel-title"><h2>{activeJob ? '处理详情与溯源' : '选择一个处理任务'}</h2>{activeJob && <div className="pipeline-detail-actions"><span>{stageLabels[activeJob.current_stage] ?? activeJob.current_stage}</span>{!['queued', 'running'].includes(activeJob.status) && <button title="删除文档及处理结果" onClick={() => onDelete(activeJob)}><Trash2/></button>}</div>}</div>{activeJob ? <><div className="pipeline-summary"><div><small>当前阶段</small><b>{stageLabels[activeJob.current_stage] ?? activeJob.current_stage}</b></div><div><small>候选对象</small><b>{activeJob.total_entities}</b></div><div><small>候选关系</small><b>{activeJob.total_relations}</b></div><div><small>模型任务</small><b>{activeJob.mineru_task_id || 'Harness'}</b></div></div><div className="pipeline-tabs"><div className="trace-list">{events.map((event, index) => { const output = preview(event.output_preview); return <details className="trace-item" key={`${String(event.stage ?? event.event)}-${index}`} open={index === events.length - 1 || Boolean(output)}><summary><span className={`trace-dot ${event.status === 'completed' ? 'done' : event.status === 'needs_review' ? 'review' : ''}`}>{event.status === 'completed' ? <Check/> : <Clock3/>}</span><div><b>{String(event.label ?? event.event ?? '运行事件')}</b><small>{event.timestamp ? new Date(String(event.timestamp)).toLocaleTimeString('zh-CN') : '已记录'}{event.model ? ` · 模型 ${String(event.model)}` : ''}{event.tool ? ` · 工具 ${String(event.tool)}` : ''}{event.entities !== undefined ? ` · ${String(event.entities)} 个对象` : ''}{event.relations !== undefined ? ` · ${String(event.relations)} 条关系` : ''}</small></div><em>{event.status === 'needs_review' ? '待确认' : event.status === 'running' ? '处理中' : event.status === 'completed' ? '完成' : ''}</em></summary>{output && <pre>{output}</pre>}</details> })}</div>{candidates?.entities?.length ? <div className="candidate-preview"><b>候选本体对象</b>{candidates.entities.slice(0, 8).map((item, index) => <div key={index}><span>{String(item.entity_type ?? 'Evidence')}</span><strong>{String(item.label ?? item.external_id ?? '未命名对象')}</strong><em>{Math.round(Number(item.confidence ?? .5) * 100)}%</em></div>)}</div> : null}</div>{activeJob.status === 'awaiting_confirmation' && <div className="review-box"><div><Sparkles/><div><b>候选本体更新已生成</b><span>请核对对象、关系和来源证据。确认后才会写入正式营销知识图谱。</span></div></div><div className="review-actions"><button className="secondary" onClick={() => onReview(activeJob, 'reject')}><XCircle/>驳回更新</button><button className="primary" onClick={() => onReview(activeJob, 'approve')}><Check/>确认并更新本体</button></div></div>}</> : <Empty icon={Eye} title="查看处理溯源" text="选择左侧任务，查看文件解析、数据处理智能体和本体更新的每一步动态。"/>}</section>
    </div>
  </>
}
...
function KnowledgeWorkspace({ graph, stats, request }: { graph: MarketingGraph; stats: GraphStats; request: <T>(path: string, init?: RequestInit, form?: boolean) => Promise<T> }) {
  ...
  return <><div className="page-head"><div><h1>知识中心</h1><p>统一管理知识文档、业务事实、本体关系和来源溯源，为智能体提供可信业务上下文</p></div><div className="knowledge-actions"><label><Search/><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && void search()} placeholder="搜索航线、运价、活动、服务规则…"/></label><button className="secondary" onClick={() => void search()}><Search/>检索知识</button></div></div><div className="knowledge-kpis"><Kpi label="知识对象" value={String(stats.entity_count)} note="统一业务语义"/><Kpi label="关系连接" value={String(stats.relation_count)} note="可解释关系"/><Kpi label="来源文档" value={String(stats.source_count)} note="可回溯证据"/><Kpi label="知识类型" value={String(Object.keys(stats.entity_types).length)} note="统一语义模型"/></div><div className="knowledge-layout"><section className="panel knowledge-graph"><div className="panel-title"><h2>本体关系视图</h2><span>知识中心 · {graph.nodes.length} 对象 · {graph.edges.length} 关系</span></div><GraphCanvas graph={graph} onSelect={() => undefined}/></section><section className="panel knowledge-results"><div className="panel-title"><h2>知识文档与检索结果</h2><span>{knowledge.length ? `${knowledge.length} 条` : '按关键词检索'}</span></div>{knowledge.map((item) => <article className="knowledge-result" key={item.chunk_id}><b>{item.title}</b><p>{item.content.slice(0, 230)}{item.content.length > 230 ? '…' : ''}</p><small><BookOpen/> {item.linked_objects.length} 个关联业务对象 · {item.chunk_id}</small></article>)}{!knowledge.length && <Empty icon={BookOpen} title="知识中心暂无内容" text="上传业务文档后，可在这里统一查看文档证据、知识片段和本体关系。"/>}</section></div></>
}
...
function FloatingCopilot({ open, messages, input, busy, onToggle, onInput, onSend, onNavigate }: { open: boolean; messages: Array<{ role: 'user' | 'assistant'; content: string; trace?: Array<Record<string, unknown>>; sources?: Array<Record<string, unknown>> }>; input: string; busy: boolean; onToggle: () => void; onInput: (value: string) => void; onSend: () => void; onNavigate: (target: string) => void }) {
  ...
  return <div className={`floating-copilot ${open ? 'open' : ''}`}>
    ...
    <button className="floating-copilot-button" title={open ? '关闭营销智能体' : '打开营销智能体'} onClick={onToggle}><span className="robot-pulse"/><Bot/><b>营销助手</b></button>
  </div>
}
