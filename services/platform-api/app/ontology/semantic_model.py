...
ATTRIBUTE_DEFINITIONS = {
    "MarketSignal": ["signal_type", "topic", "occurred_at", "source", "confidence"],
    "Market": ["market_scope", "region", "country_region", "season", "status"],
    "Airport": ["airport_code", "city", "country_region", "airport_type", "status"],
    "Route": ["route_code", "origin", "destination", "market_scope", "distance_km"],
    "Flight": ["flight_no", "flight_date", "route_code", "departure_time", "operation_status"],
    "FlightSegment": ["flight_no", "origin", "destination", "departure_at", "arrival_at"],
    "Cabin": ["cabin_code", "inventory", "available_seats", "fare_basis", "status"],
    "Fare": ["fare_basis", "price", "passenger_type", "refund_rule", "change_rule"],
    "ProductLabel": ["label_code", "label_name", "source_system", "value_type", "status"],
    "ProductGroup": ["group_code", "group_name", "category", "product_count", "status"],
    "AncillaryProduct": ["product_code", "service_type", "price", "availability", "validity"],
    "MetricObservation": ["metric_code", "metric_value", "unit", "observed_at", "source"],
    "Opportunity": ["opportunity_type", "signal_ids", "route_id", "target_need", "score", "status"],
    "MarketingObjective": ["objective_type", "target_value", "measurement_window", "owner", "status"],
    "CustomerNeed": ["need_type", "journey_stage", "price_sensitivity", "service_preference", "evidence"],
    "CustomerAggregate": ["audience_type", "member_level", "travel_frequency", "attributes", "population"],
    "ConfigurableAttribute": ["attribute_code", "attribute_name", "value_type", "source", "validity"],
    "AudienceSnapshot": ["snapshot_code", "segment_id", "selection_logic", "population", "frozen_at"],
    "Product": ["product_code", "product_type", "price", "eligibility", "validity"],
    "ProductPackage": ["package_code", "version", "components", "eligibility", "approval_status"],
    "ValueProposition": ["proposition", "target_need", "differentiator", "evidence_ids", "status"],
    "StrategyPlan": ["plan_code", "objective_id", "audience_id", "product_package_id", "budget", "frequency_rule"],
    "TouchpointPlan": ["channel", "trigger", "time_window", "frequency_limit", "content_id"],
    "ContentAsset": ["content_id", "channel", "title", "body", "version", "compliance_status"],
    "MarketingCase": ["case_code", "opportunity_id", "owner", "stage", "status"],
    "Campaign": ["campaign_id", "name", "objective", "budget", "owner", "status"],
    "CampaignVersion": ["version", "audience_snapshot_id", "product_package_id", "content_asset_ids", "channels", "status"],
    "ApprovalTask": ["approval_type", "approver_role", "decision", "comment", "decided_at"],
    "ExecutionBatch": ["batch_code", "channel", "target_count", "sent_count", "status"],
    "Channel": ["channel_code", "channel_type", "provider", "consent_required", "status"],
    "Feedback": ["delivered_count", "clicked_count", "converted_count", "unsubscribe_count", "complaint_count"],
    "AttributionResult": ["attribution_model", "incremental_orders", "incremental_revenue", "roi", "confidence"],
    "Review": ["review_period", "conclusion", "key_findings", "next_action", "status"],
    "Recommendation": ["recommendation_type", "reasoning", "confidence", "status", "confirmed_by"],
    "BusinessRule": ["rule_type", "expression", "priority", "effective_period", "status"],
    "Evidence": ["source_type", "source_ref", "excerpt", "captured_at", "confidence"],
    "HumanDecision": ["decision_type", "operator", "decision", "comment", "decided_at"],
    "AgentRun": ["domain_id", "input_context", "output_summary", "status", "created_at"],
    "KnowledgeDocument": ["document_id", "title", "source_name", "file_type", "version"],
    "KnowledgeChunk": ["document_id", "page_no", "paragraph_no", "content_hash"],
    "KnowledgeClaim": ["claim_type", "subject_id", "predicate", "object_id", "confidence"],
}


RELATION_TYPES = [
    {"id": "departs_from", "name": "出发机场", "from_types": ["Route", "FlightSegment"], "to_types": ["Airport"]},
    {"id": "arrives_at", "name": "到达机场", "from_types": ["Route", "FlightSegment"], "to_types": ["Airport"]},
    {"id": "has_segment", "name": "包含航段", "from_types": ["Flight"], "to_types": ["FlightSegment"]},
    {"id": "operates_on", "name": "执飞航线", "from_types": ["Flight"], "to_types": ["Route"]},
    {"id": "has_cabin", "name": "提供舱位", "from_types": ["Flight", "FlightSegment"], "to_types": ["Cabin"]},
    {"id": "has_fare", "name": "提供运价", "from_types": ["Cabin"], "to_types": ["Fare"]},
    {"id": "uses_product", "name": "引用产品", "from_types": ["Fare"], "to_types": ["Product", "AncillaryProduct"]},
    {"id": "has_product_label", "name": "挂接产品标签", "from_types": ["Product", "ProductPackage", "Flight", "FlightSegment"], "to_types": ["ProductLabel"]},
    {"id": "includes_ancillary", "name": "包含辅营", "from_types": ["Fare", "ProductPackage", "Product"], "to_types": ["AncillaryProduct"]},    {"id": "derived_from", "name": "来源于", "from_types": ["MarketSignal", "MetricObservation", "Opportunity", "Recommendation", "MarketingCase"], "to_types": ["Evidence", "MarketSignal", "MetricObservation", "Review"]},
    ...
    {"id": "belongs_to_product_group", "name": "属于产品组", "from_types": ["Product"], "to_types": ["ProductGroup"]},
...
def semantic_model() -> dict:
    return {
        ...
        "object_types": [dict(item, attributes=ATTRIBUTE_DEFINITIONS.get(item["id"], ["id", "name", "source", "status"])) for item in OBJECT_TYPES],
